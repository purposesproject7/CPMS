...
const AdminPanelManagement = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const panelsFormatted = panelRes.data.data.map((p) => ({
        facultyIds: [p.faculty1._id, p.faculty2._id],
        facultyNames: [p.faculty1.name, p.faculty2.name],
        panelId: p._id,
      }));
  ...
  const handleAddPanel = async () => {
    ...
    const exists = panels.find(p => p.facultyIds.includes(f1) && p.facultyIds.includes(f2));
  ...
  const filterMatches = (str) => str.toLowerCase().includes(searchQuery.toLowerCase());
        ...
        <div className="p-20 pl-28">
          <div className=" shadow-md rounded-lg bg-white p-10 ">
            ...
            <div className="mb-4 flex flex-wrap gap-4 items-center">
              ...
              {/* <button onClick={() => setShowDebug(!showDebug)} className="bg-gray-700 text-white px-3 py-1 rounded">
