...
const FacultyManagement = () => {
  ...
  const createFaculty = async (facultyData) => {
    // Mock API call for demo - replace with actual API endpoint
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve({ message: 'Faculty created successfully!' });
      }, 1500);
    });
  };
  ...
  const handleSubmit = async () => {
    // Clear inputs immediately when button is clicked
    ...
    try {
      ...
      if (!currentFormData.employeeId.trim()) {
        throw new Error('Employee ID is required');
      }
      
      if (!currentFormData.employeeId.match(/^[A-Za-z0-9]+$/)) {
        throw new Error('Employee ID must contain only letters and numbers');
      }
      
      if (!currentFormData.emailId.trim()) {
        throw new Error('Email is required');
      }
      
      if (!currentFormData.emailId.endsWith('@vit.ac.in')) {
      ...
      if (!/[A-Z]/.test(currentFormData.password)) {
      ...
      if (!/[0-9]/.test(currentFormData.password)) {
        throw new Error('Password must contain at least one number');
      }
      
      if (!/[^A-Za-z0-9]/.test(currentFormData.password)) {
        throw new Error('Password must contain at least one special character');
      }

      // Prepare data for API call (excluding imageUrl as backend doesn't expect it)
      const apiData = {
        name: currentFormData.name.trim(),
        emailId: currentFormData.emailId.trim(),
        password: currentFormData.password,
        employeeId: currentFormData.employeeId.trim()
      };

      // Call appropriate API based on role
      let response;
      if (currentFormData.role === 'faculty') {
        response = await createFaculty(apiData);
      } else {
        response = await createAdmin(apiData);
      }

      setSuccess(response.message || `${currentFormData.role === 'faculty' ? 'Faculty' : 'Admin'} created successfully!`);
